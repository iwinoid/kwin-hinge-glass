...
struct Config
{
    ...
    /// 强度曲线指数。θ = 上限 × (折了多少 / 原角度)^指数
    ///
    /// 1.0 = 线性，开头涨得慢，要折下去好几度才看得见。
    /// 小于 1 时开头涨得快、中段更强，而完全合上时仍是满效果。
    /// 实测平均起效延迟：1.0 → 370ms，0.8 → 249ms，0.65 → 192ms，0.5 → 149ms。
    /// 取 0.65 是因为再往下开头会显得突兀。
    double curveExponent = 0.65;
