...
| 折叠角上限 | 45° | 比原角度再低这么多时达到满效果。 |
| 最小角度变化 | 10° | 触发特效所需的最小角度变化。传感器噪声有几度，取值过低会被碰桌子触发。 |
...
| 空闲采样率 | 10 Hz | 屏幕静止时的传感器采样率。 |
| 活动采样率 | 30 Hz | 屏幕移动时的传感器采样率。 |
| 角度平滑弹簧 | 16 rad/s | 把传感器读数平滑到屏幕刷新率。数值越大越跟手。 |
...
- **1.0**：首个版本。含转轴角度传感器、玻璃着色器、状态机、设置面板、离线预览工具。
